...
if(empty($_SESSION['user_id'])) {
    ...
    if(isset($_POST['getitemTable'])){
        $itemName = $_POST['itemName'];


        // Query to check if the item exists in the user's orders
		
        
		$getItemQuery = mysqli_query($db,"SELECT * FROM users_orders WHERE u_id = '".$_SESSION['user_id']."' AND title = '$itemName'");
        $row = mysqli_fetch_assoc($getItemQuery);
        $dateOrdered = $row['date'];
		
        if (mysqli_num_rows($getItemQuery) > 0 ) {
            echo "Yes, you have ordered this item on $dateOrdered.";
        } else {
            echo "No matches found for this item in your orders.";
        }
    }
...
<body>
        ...
        <div class="page-wrapper">
			...
			<!-- Add this input field and button inside your HTML form -->
			<form method="post" class="item-form">
        <label for="itemName">Enter Item Name:</label>
        <input type="text" name="itemName" required>
        <input type="submit" name="getitemTable" value="Get Item Details">
    </form>
